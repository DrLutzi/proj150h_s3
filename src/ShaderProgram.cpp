...
void ShaderProgram::load(const std::string& vert_name, const std::string& frag_name, const std::string& geom_name)
{
    ...
    //Création du geometry shader
    m_geomShader = new Shader(GL_GEOMETRY_SHADER);
    m_geomShader->compileShader(geom_name);
    ...
    glDetachShader(m_programId, m_geomShader->shaderId());
